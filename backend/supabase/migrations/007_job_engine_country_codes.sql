...
-- Word boundaries only — never unanchored city aliases (`gent` ⊂ `argentina`).
UPDATE job_postings
SET country_code = CASE
    WHEN location ~* '\y(argentina|buenos aires)\y' THEN 'AR'
    WHEN location ~* '\y(belgium|belgique|belgi[eë]|brussels|bruxelles|brussel|ghent|antwerp|antwerpen)\y' THEN 'BE'
    WHEN location ~* '\ygent\y' THEN 'BE'
    WHEN location ~* '\y(netherlands|nederland|holland|amsterdam|rotterdam|utrecht|eindhoven|den haag|the hague)\y' THEN 'NL'
    WHEN location ~* '\y(luxembourg|luxemburg)\y' THEN 'LU'
    WHEN location ~* '\y(france|paris|lyon|lille|marseille|toulouse|nantes|bordeaux|île-de-france|ile-de-france)\y' THEN 'FR'
    WHEN location ~* '\y(germany|deutschland|berlin|munich|münchen|hamburg|frankfurt|cologne|köln)\y' THEN 'DE'
    WHEN location ~* '\y(united kingdom|great britain|england|london|manchester|edinburgh)\y' OR location ~* '\yuk\y' THEN 'GB'
    WHEN location ~* '\y(ireland|dublin)\y' OR location ~* '\yeire\y' THEN 'IE'
    WHEN location ~* '\y(spain|españa|espana|madrid|barcelona)\y' THEN 'ES'
    WHEN location ~* '\y(italy|italia|rome|roma|milan|milano)\y' THEN 'IT'
    WHEN location ~* '\y(portugal|lisbon|lisboa)\y' THEN 'PT'
    WHEN location ~* '\y(switzerland|schweiz|suisse|zurich|zürich|geneva|genève)\y' THEN 'CH'
    WHEN location ~* '\y(austria|österreich|vienna|wien)\y' THEN 'AT'
    WHEN location ~* '\y(sweden|sverige|stockholm)\y' THEN 'SE'
    WHEN location ~* '\y(norway|norge|oslo)\y' THEN 'NO'
    WHEN location ~* '\y(denmark|danmark|copenhagen)\y' THEN 'DK'
    WHEN location ~* '\y(poland|polska|warsaw|warszawa)\y' THEN 'PL'
    WHEN location ~* '\y(united states|new york|san francisco|seattle|austin|boston|chicago|los angeles)\y' OR location ~* '\y(usa|nyc)\y' THEN 'US'
    WHEN location ~* '\y(canada|toronto|montreal|vancouver)\y' THEN 'CA'
    WHEN location ~* '\y(australia|sydney|melbourne)\y' THEN 'AU'
    WHEN location ~* '\ysingapore\y' THEN 'SG'
    WHEN location ~* '\y(india|bangalore|bengaluru|hyderabad|mumbai)\y' THEN 'IN'
