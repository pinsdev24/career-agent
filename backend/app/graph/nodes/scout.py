...
async def scout_node(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    await log_emitter.emit(
        state.get("run_id"),
        {"type": "info", "message": "Scout: Initiating web search for job offers..."},
    )
    ...
    await log_emitter.emit(
        ...
        {
            "type": "agent_action",
            "message": f"Scout querying Tavily specific job boards: '{query[:80]}...'",
        },
    )
    ...
    await log_emitter.emit(
        ...
        {
            "type": "info",
            "message": f"Scout: Found {len(filtered_results)} valid individual job postings ({len(raw_results)} total raw).",
        },
    )
    await log_emitter.emit(
        state.get("run_id"),
        {"type": "info", "message": "Scout: Generating embeddings to pre-score semantic match..."},
    )
    ...
    await log_emitter.emit(
        state.get("run_id"),
        {"type": "info", "message": f"Scout: Found and scored {len(offers)} offers."},
    )
