...
class LetterGrade(TypedDict):
    ...
    reasoning: Annotated[str, "Explain reasoning regarding grammar, tone, and accuracy of skills compared to CV profile."]
    is_professional: Annotated[bool, "True if the letter is professional and grammatically correct."]
    is_hallucination_free: Annotated[bool, "True if the letter does NOT invent skills/experiences not present in the CV."]
...
async def evaluate_letter_quality(inputs: dict, outputs: dict, reference_outputs: dict) -> dict:
    ...
    grader_llm = ChatOpenAI(model="gpt-5-mini", temperature=0).with_structured_output(LetterGrade, strict=True)
